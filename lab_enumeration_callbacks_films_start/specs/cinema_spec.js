...
describe('Cinema', function () {
  ...
  it('should be able to check whether there are some films from a particular year', function() {
    ...
    // assert.deepStrictEqual(actual, expected);
    // assert.deepStrictEqual(cinema.findFilmsByYear(1999), []);
    const actual = cinema.doesHaveFilmsFromYear(2017);
    assert.strictEqual(actual, true);

  });

  it('should be able to check whether there are no films from a particular year', function() {
    const actual = cinema.doesHaveFilmsFromYear(1999);
    assert.strictEqual(actual, false);
  });

  it('should be able to check whether all films are over a particular length', function() {
    const actual = cinema.checkIfAllFilmsAreLongerThan(50);
    assert.strictEqual(actual, true);

    const actual2 = cinema.checkIfAllFilmsAreLongerThan(120);
    assert.strictEqual(actual2, false);

    const actual3 = cinema.checkIfAllFilmsAreLongerThan(200);
    assert.strictEqual(actual2, false);
  });
